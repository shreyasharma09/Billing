...
const AllProductsComponent = () => {
   ...
   const getallproducts=async(token)=>{
    try {
        const response=await fetch("https://quickbill-1.onrender.com/api/getproducts",{
            headers:{
   ...
   const deleteproduct=async(id)=>{
    try {
        ...
        const response=await fetch("https://quickbill-1.onrender.com/api/deleteproduct/"+id,{
            method:"delete",
            headers:{
                "Content-Type":"application/json",
                                ...
                                <div className="card">
                                    <div className="card-body">
                                        ...
                                        <div className="row align-items-center mb-2 gy-3">
                                            ...
                                            <p className="mb-0 text-muted">Showing <b>{(currentpage - 1) * productsperpage + 1}</b> to {" "}<b>{Math.min(currentpage * productsperpage, data.length)}</b>{" "} of <b>{data.length}</b> results</p>                                            </div>
                                            <div className="col-sm-auto ms-auto">
                                            <nav aria-label="...">
