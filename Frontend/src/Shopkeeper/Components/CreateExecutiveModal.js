...
const CreateExecutiveModal = (props) => {
    const[displayotpsection,setdisplayotpsection]=useState(false)   //popup
    const[otploading,setotploading]=useState(false)
    const[loading,setloading]=useState(false)
    const[obj,setobj]=useState({})
    const[otp,setotp]=useState('')
    const[data,setdata]=useState({}) //for state city
    ...
    const sendotp=async(e)=>{
        try {
            ...
            const response=await fetch("https://quickbill-1.onrender.com/api/verifyexecutive",{
                method:"post",
    ...
    const submit=async(e)=>{
        try {
            ...
            const response=await fetch("https://quickbill-1.onrender.com/api/createexecutive",{
                method:"post",
