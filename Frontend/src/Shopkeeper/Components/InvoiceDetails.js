...
const InvoiceDetails = ({data}) => {
    ...
    FutureDate.setDate(today.getDate() + 7)     //due date
   
    const getCustomer=async(token,id)=>{
    try {
        const response=await fetch("https://quickbill-1.onrender.com/api/getCustomer/"+id,{
            headers:{
    ...
    try {
        const response=await fetch("https://quickbill-1.onrender.com/api/getShopkeeper",{
            headers:{
                "Content-Type":"application/json",
                "Authorization":token
            }
